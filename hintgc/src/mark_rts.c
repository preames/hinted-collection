...
{
    while (traced_stack_sect != NULL) {
      if( !(lo HOTTER_THAN (ptr_t)traced_stack_sect) ) {
        // PREAMES
        // seems to be a preexisting bug, but not sure.  Nor do
        // I care.  I simply removed this portion from the test suite
        // since non of the benchmark use this advanced feature.
        // This is hit only by using do_blocking, call_with_gc_active
        GC_printf("ERROR\n");
        GC_printf("%p\n", lo);
        GC_printf("%p\n", hi);
