...
# define CLEAR_SIZE 213  /* Granularity for GC_clear_stack_inner */
  ...
  void * GC_clear_stack_inner(void *arg, ptr_t limit)
  {
    ...
    //    if(!((word)GC_approx_sp() COOLER_THAN (word)limit)) return(arg);
...
// Comment one of these out to sample the given event counters.  
// Some combinations don't work together.  For example, on my 
// laptop, can't include L2, L3 in the same sample set. 
// Same with the total cycles, duplicate codes,
//STATIC int papi_events[] = {PAPI_STL_ICY}; //cylces w/no instructions
//STATIC int papi_events[] = {PAPI_TOT_INS};  //total instruction PF << GC
STATIC int papi_events[] = {PAPI_TOT_CYC};  //total cylces
//STATIC int papi_events[] = {PAPI_BR_CN, PAPI_BR_NTK, PAPI_BR_MSP};  
//STATIC int papi_events[] = {PAPI_VEC_SP, PAPI_VEC_DP};  //no fp vect op
//STATIC int papi_events[] = {PAPI_L1_TCM, PAPI_L1_LDM, PAPI_L1_STM};//, PAPI_L2_TCM, PAPI_L3_TCM};
//STATIC int papi_events[] = {PAPI_L2_TCM, PAPI_L3_TCM}; //can't get at same time as L1, GRR...
//STATIC int papi_events[] = {PAPI_L3_TCR, PAPI_L3_TCW}; //L3 traffic

//auto size an integer length
STATIC const int papi_event_size = sizeof(papi_events)/sizeof(int);

//larger than any papi_events
STATIC long long papi_values[30];
#endif //BUILD_WITH_PAPI
...
#   endif
    ...
    status = PAPI_read_counters(papi_values, papi_event_size);
...
#        endif
         ...
         // There looks to be a race condition on updates to errno here.  Thus,
         // we can NOT do any conditional check on errno.  Joy.
         //         GC_ASSERT( (errno == EINVAL) ); //preames - check for sanity
         bytes_written += result;
      }
      if( (size_t)bytes_written != len ) abort();
      RESTORE_CANCEL(cancel_state);
...
/// Do a buffered printf style write to the given file descriptor.
...
{
  // As a hack, we we run the SPEC benchmarks, we do it
  // with a version of the collector modified to not print anything.  
  // this is needed to prevent our debug output from causing SPEC to
  // supriously fail.
#if 1
    va_list args;
    char buf[BUFSZ+1];
...
#ifdef BUILD_WITH_DBGUTILS
      //PREAMES - Never returns
    dbgutils_abort_with_backtrace();
#endif //BUILD_WITH_DBGUTILS
