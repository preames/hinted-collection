...
void show_backtrace (void) {
  ...
  dbgutils_print_backtrace();
#endif
  GC_printf("Printing backtraces unsupported on this platform\n");
}

void exit_with_backtrace (int status) {
  GC_log_printf("Exitting program with status: %d\n", status);
  GC_printf("Printing backtraces unsupported on this platform\n");
#ifdef BUILD_WITH_DBGUTILS
  dbgutils_exit_with_backtrace(status);
#else //BUILD_WITH_DBGUTILS
  exit(status);
