...
#error "Does not match expectation..."
#endif
...
#   define GC_bytes_pending_free GC_arrays._bytes_pending_free
  word _bytes_pending_free;

  //guard addresses
#   define GC_pending_free_high GC_arrays._pending_free_high
  ptr_t _pending_free_high;
#   define GC_pending_free_low GC_arrays._pending_free_low
  ptr_t _pending_free_low;
...
#include "hintgc_config.h"
#ifdef BUILD_WITH_DBGUTILS
#include "dbgutils.h"
#define GC_ASSERT(expr) DBGUTILS_ASSERT(expr)
#else
#  define GC_ASSERT(expr) \
                if (!(expr)) { \
                  GC_err_printf("Assertion failure: %s:%d\n", \
