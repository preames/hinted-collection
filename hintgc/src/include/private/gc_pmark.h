...
/// BEGIN PENDING FREE HACKERY

// copied from PUSH_CONTENTS, modified to only mark objects
// which live in pending free blocks and not push the contents of any

// This is where the edge filtering code lives.  Currently,
// only the header-check version is enabled.  This is what most
// of my experimentation was done with.  Micro benchmark results
// seem to indicate that running both range-filter and 
// header-check (in that order) might be profitable overall.  
// It might be worth playing with this more.

/* header-check version                                             \
   if( !(my_hhdr->hb_flags & HAS_PENDING_FREE) ) {                  \
      goto exit_label;                                              \
   }  
*/

/* range-check version
  if( GC_pending_free_low > current ||                              \
  current > GC_pending_free_high  ) {                               \
    goto exit_label;                                                \
  }                                                                 \
*/
#define MARK_NO_PUSH(current, mark_stack_top, mark_stack_limit,     \
                     source, exit_label)                            \
  {                                                                 \
    hdr * my_hhdr;                                                  \
    HC_GET_HDR(current, my_hhdr, source, exit_label);               \
    ...
    MARK_NO_PUSH_HDR(current, mark_stack_top, mark_stack_limit,     \
                     source, exit_label, my_hhdr, TRUE);            \
  exit_label: ;                                                     \
  }

//copied from PUSH_CONTENTS_HDR
# define MARK_NO_PUSH_HDR(current, mark_stack_top, mark_stack_limit, \
                           source, exit_label, hhdr, do_offset_check) \
{ \
    size_t displ = HBLKDISPL(current); /* Displacement in block; in bytes. */\
    /* displ is always within range.  If current doesn't point to       */ \
    ...
    GC_STORE_BACK_PTR((ptr_t)source, base); \
    /* 
       We only want to mark this object.  Do not, repeat, do not 
       push any of it's children
       PUSH_OBJ(base, hhdr, mark_stack_top, mark_stack_limit);   */ \
}

/// END PENDING FREE HACKERY
